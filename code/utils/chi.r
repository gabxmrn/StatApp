...
data_us_new_var <- data_us_new_var %>%
  mutate(across(c(chomage, taux_ct, conso_growth, spread,confiance,income_growth), ~ lead(.x, n = 2), .names = "lag2_{.col}"))

data_us_new_var <- data_us_new_var %>%
  mutate(across(c(chomage, taux_ct, conso_growth,spread , confiance,income_growth,delta_log_c), ~ lead(.x, n = 1), .names = "lag1_{.col}"))
...
model_IV <- lm(lag1_delta_log_c ~ lag2_chomage + lag2_taux_ct + lag2_spread + lag2_conso_growth + lag2_confiance + lag2_income_growth, data = data_us_new_var, na.action = na.omit)

#reconstruction de la prédiction sur les IV
new_data <- data_us_new_var %>%
  select(lag1_chomage ,lag1_taux_ct,lag1_spread,lag1_conso_growth, lag1_confiance, lag1_income_growth) %>%  # Sélectionner les colonnes
  rename(lag2_chomage = lag1_chomage, lag2_taux_ct = lag1_taux_ct, lag2_spread = lag1_spread, lag2_conso_growth = lag1_conso_growth, lag2_confiance = lag1_confiance, lag2_income_growth = lag1_income_growth)
